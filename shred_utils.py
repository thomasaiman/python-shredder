...
def scn_xcorr(slidePtr, SR, manual_img):
    ...
    manual_gray = manual_img.transpose(Image.ROTATE_90).convert(mode='F') #uses perceptual luminance weighted sum to convert RGB to F
...
def region_selector(slice_info, slidePtr=None):
    ...
    if stain == 'myelin':
        ...
        raise ValueError('Need to pick an error threshold for accepting regions of this stain')
        
    for v in SR:
        if v['mean_sq_err'] < err_threshold:
            v['in_slice'] = True
        else:
            v['in_slice'] = False
...
def extractor(slidePtr,SR,level,bounds, bg_color):
    """
    ...
    #lm_chunk will be oversized by 2 so there is a pad of blank pixels around it
    #this saves much time from copying/resizing during the bg_color fill
    lm_chunk = np.zeros([rmax-rmin+1+2, cmax-cmin+1+2, 3], dtype='uint8') 
    for s in SR:
        ...
        if s['in_chunk']:
            ...
            #removes all pure black and pure white areas connected to the edge
            cvedgefill(lm_chunk, bg_color)
            
    return(lm_chunk[1:-1, 1:-1])        
        




def cvedgefill(im, bg_color):
    #im should be padded so that it has a ring of black pixels around it
    im[0 , :, :] = 0
    im[-1, :, :] = 0
    im[: , 0, :] = 0
    im[:, -1, :] = 0
    h,w = im.shape[0:2]
    #cv.floodFill works in place on the image, but requires a blank mask array
    mask=np.zeros((h+2, w+2), dtype='uint8')
    cv.floodFill(im, mask, seedPoint=(0,0), newVal=bg_color)
