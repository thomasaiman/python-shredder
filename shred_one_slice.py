...
def shred_one_slice(slice_info):
    ...
    df_R2 = cv.resize(df_R, dsize=(4096,4096), interpolation=cv.INTER_LANCZOS4)
    df_C2 = cv.resize(df_C, dsize=(4096,4096), interpolation=cv.INTER_LANCZOS4)
    df_R2 = df_R2*16
    df_C2 = df_C2*16
    
    manual_img=np.asarray(manual_img)
    lev2_deformed = cv.remap(manual_img,
                                df_C2,
                                df_R2,
    ...
    for level in (1,):
        ...
        for ii in range(1,num_rows+1):
            ...
            for jj in range(1,num_cols+1):
                ...
                pre_tile = extractor(slidePtr, SR, level, bounds, bg_color)
                
                if pre_tile is None:
                    shutil.copy(blankPath, imgPath)
                else:
                    #rotate to the correct orientation
                    pre_tile = np.rot90(pre_tile,-1)
                    ...
                    #cv.remap(src, map_x, map_y)
                    tile = cv.remap(pre_tile,
                                    df_Cc,
                                    df_Rc, 
                                    interpolation=cv.INTER_LANCZOS4, 
                                    borderMode=cv.BORDER_CONSTANT, 
